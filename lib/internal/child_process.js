  ...
  'net.Server': {
    ...
    send(message, server, options) {
      ...
      ArrayPrototypeForEach(serverOptions, (option) => {
        console.log('original option', option, server[option])
        server[option] !== undefined && (message.serverConfig[option] = server[option]);
      })
      // revert keepAliveInitialDelay unit
      server.keepAliveInitialDelay !== undefined && (message.serverConfig.keepAliveInitialDelay = server.keepAliveInitialDelay * 1000);
...
function setupChannel(target, channel, serializationMode) {
  ...
  target._send = function (message, handle, options, callback) {
    ...
    if (handle) {
      ...
      // todo: has a bug that it can't convert net.server object correctly
