...
function pushAsyncContext(asyncId, triggerAsyncId, resource) {
  debugger
  const buffer = `buffer:${asyncId}:${triggerAsyncId}`
  binding.writeString(1, buffer, 0, buffer.length,
    undefined, {});
  // fs.writeSync(1, `pushAsyncContext:${asyncId}:${triggerAsyncId}\n`);
  ...
  // 将 kExecutionAsyncId 和 kTriggerAsyncId 存入栈
...
function popAsyncContext(asyncId) {
  debugger
  // fs.writeSync(1, `popAsyncContext:${asyncId}\n`);
